...
class Game extends Component {
  ...
  getFilteredImages = () => {
    ...
    const filteredImages = imagesList.filter(
      eachImage => eachImage.category === currentTab,
    )
  ...
  onClickAnimalsBtn = () => {
    this.setState({
      currentTab: tabsList[1].tabId,
    })
  }

  onClickPlacesBtn = () => {
    this.setState({
      currentTab: tabsList[2].tabId,
    })
  }

  onClickImg = id => {
    const {randomNum} = this.state
    const randomId = imagesList[randomNum].id
    if (randomId === id) {
  ...
  renderImages = () => {
    const {randomNum} = this.state
    const updatedImages = this.getFilteredImages()
    const randomImg = imagesList[randomNum].imageUrl
    return (
      <>
        <div className="random-img-section">
          <img src={randomImg} alt="match" className="random-image" />
        </div>
        <div className="btns-section">
          <button
            type="button"
            className="fruitsBtn"
            onClick={this.onClickFruitsBtn}
          >
            Fruits
          </button>
          <button
            type="button"
            className="animalsBtn"
            onClick={this.onClickAnimalsBtn}
          >
            Animals
          </button>
          <button
            type="button"
            className="placesBtn"
            onClick={this.onClickPlacesBtn}
          >
            Places
          </button>
        </div>
        <ul className="images-section">
          {updatedImages.map(eachImage => (
            <ImageItem
              key={eachImage.id}
  ...
  onClickRetry = () => {
    this.setState({gameOver: false, score: 0, time: 60})
  ...
  render() {
    const {score, time, gameOver} = this.state
    return (
      <div className="main-container">
        ...
        <div className="game-section">
          {gameOver ? this.renderGameOver() : this.renderImages()}
        </div>
      </div>
    )
  }
